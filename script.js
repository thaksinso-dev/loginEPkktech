...
const CONFIG = {
    ...
    // ==========================================================
    // LOCAL LOGIN (ไม่ต้องพึ่ง Google Apps Script)
    // แก้ไข/เพิ่มบัญชีเริ่มต้นได้ที่นี่ ผู้ใช้ที่เพิ่มผ่านหน้า "จัดการผู้ใช้"
    // จะถูกเก็บเพิ่มเติมใน LocalStorage ของเบราว์เซอร์นี้เท่านั้น
    // ==========================================================
    DEFAULT_USERS: [
...
async function handleLogin(e) {
    ...
    // หน่วงเวลาเล็กน้อยให้ดูเป็นธรรมชาติ (ไม่มีการเรียก server ใดๆ)
    setTimeout(() => {
        const found = getLocalUsers().find(u => u.username === username && u.password === password);

        if (!found) {
            showLoginResult('ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง', 'error');
        } else if (found.status !== 'active') {
            showLoginResult('บัญชีนี้ถูกปิดใช้งาน', 'error');
        } else {
            const user = {
...
// ============================================
// LOCAL USER DATABASE (แทน Google Sheet USERS)
// ============================================
function initLocalUsers() {
    try {
        if (!localStorage.getItem('ems_users')) {
            localStorage.setItem('ems_users', JSON.stringify(CONFIG.DEFAULT_USERS));
        }
    } catch (e) {}
}
function getLocalUsers() {
    try {
        return JSON.parse(localStorage.getItem('ems_users')) || CONFIG.DEFAULT_USERS;
    } catch (e) {
        return CONFIG.DEFAULT_USERS;
    }
}
function saveLocalUsers(users) {
    try { localStorage.setItem('ems_users', JSON.stringify(users)); } catch (e) {}
}
...
async function handleAddUser(e) {
    ...
    if (users.some(u => u.username === data.username)) {
        showAddUserResult('ชื่อผู้ใช้นี้มีอยู่แล้ว', 'error');
        return;
    }
    users.push(data);
